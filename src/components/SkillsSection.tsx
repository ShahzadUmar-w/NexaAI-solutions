...
const SkillsSection = () => {
  return (
    <section id="skills" className="relative overflow-hidden border-y border-white/10 bg-white/[0.018] py-24">
        ...
        <div className="grid gap-px overflow-hidden rounded-3xl border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-3">
          {skillGroups.map((skill, index) => (
            <motion.article
              ...
              className="group relative overflow-hidden bg-background/80 p-6 text-left backdrop-blur-xl transition-colors hover:bg-white/[0.045]"
            >
              <div className="relative z-10">
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.045] text-orange-200">
                ...
                <div className="flex flex-wrap gap-2">
                  {skill.tools.map((tool) => (
                    <span
                      key={tool}
                      className="rounded-full border border-white/10 bg-white/[0.045] px-3 py-1 text-xs font-medium text-orange-100"
                    >
                      {tool}
                    </span>
                  ))}
